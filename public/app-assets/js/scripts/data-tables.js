...
$(function () {
  ...
  // Page Length Option Table — stateSave preserves search/page/sort on back navigation

  // if (!$.fn.DataTable.isDataTable('#page-length-option')) {
  //   $('#page-length-option').DataTable({
  //     responsive: true,
  //     stateSave: true, // persists search, pagination & sort in sessionStorage
  //     stateDuration: -1, // -1 = session only, clears when tab/browser is closed
  //     lengthMenu: [
  //       [10, 25, 50, -1],
  //       [10, 25, 50, 'All'],
  //     ],
  //   });
  // }
  // Page Length Option Table — preserve search after back
$('#page-length-option').DataTable({
  responsive: true,
  stateSave: true,
...
$(window).on('load', function () {
  $('.dropdown-content.select-dropdown li').on('click', function () {
    ...
    setTimeout(function () {
      if (
        $(that).parent().parent().find('.select-dropdown').hasClass('active')
      ) {
        $(that)
          .parent()
          .parent()
