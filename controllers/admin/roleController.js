...
// Turn the posted checkbox form data into a clean permissions array,
// one entry per module, always in the same order as config/modules.js.
function buildPermissions(body) {
  return modules.map((m) => ({
    module: m.key,
    view: body[`perm_${m.key}_view`] === 'on',
    add: body[`perm_${m.key}_add`] === 'on',
    edit: body[`perm_${m.key}_edit`] === 'on',
    // Delete permission can never be granted for country-scoped modules
    // (User Management / CV Upload / CV Search) — that stays Super-Admin-only.
    delete: m.countryScoped ? false : body[`perm_${m.key}_delete`] === 'on',
  }));
}
...
exports.postAddRole = async (req, res) => {
  try {
    ...
    if (!name || !name.trim()) {
      req.flash('red', 'Please enter a role name.');
      return res.redirect('/roles/add');
    }

    const permissions = buildPermissions(req.body);
    const hasAnyPermission = permissions.some(
      (p) => p.view || p.add || p.edit || p.delete
    );
    if (!hasAnyPermission) {
      req.flash('red', 'Please select at least one permission before creating the role.');
      return res.redirect('/roles/add');
    }
    ...
    if (error.code === 11000) req.flash('red', 'A role with this name already exists.');
    else req.flash('red', error.message);
    res.redirect('/roles/add');
  }
};
...
exports.getEditRole = async (req, res) => {
  try {
    ...
    // Merge module list with the role's saved permissions for easy checkbox rendering.
    const permMap = {};
    role.permissions.forEach((p) => (permMap[p.module] = p));
    const modulesWithPerms = modules.map((m) => ({
      ...m,
...
exports.getDeleteRole = async (req, res) => {
  try {
    const inUse = await Admin.countDocuments({ role: req.params.id });
    if (inUse > 0) {
      req.flash('red', `Cannot delete: ${inUse} admin(s) are currently assigned this role.`);
      return res.redirect('/roles');
    }
